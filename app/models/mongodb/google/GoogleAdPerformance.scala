...
class GoogleAdPerformance extends GoogleAdGroupPerformance{
  ...
  override def fromDocument(doc: Document){
    ...
    this.parseStringField(doc, adNameField.fieldName, adName)
    this.parseLongField(doc, adIdField.fieldName, adId)
    this.parseStringField(doc, adStateField.fieldName, adState)
    this.parseStringField(doc, adTypeField.fieldName, adType)
    this.parseStringField(doc, adDescriptionField.fieldName, adDescription)
    this.parseStringField(doc, adDescriptionLine1Field.fieldName, adDescriptionLine1)
    this.parseStringField(doc, headline1Field.fieldName, headline1)
    this.parseStringField(doc, headline2Field.fieldName, headline2)
    this.parseStringField(doc, longHeadlineField.fieldName, longHeadline)
    this.parseStringField(doc, imageAdNameField.fieldName, imageAdName)
    this.parseStringField(doc, imageWidthField.fieldName, imageWidth)
    this.parseStringField(doc, imageHeightField.fieldName, imageHeight)
    this.parseStringField(doc, path1Field.fieldName, path1)
    this.parseStringField(doc, path2Field.fieldName, path2)
    this.parseStringField(doc, adHtmlField.fieldName, adHtml)
  }
}
