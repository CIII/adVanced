...
abstract class GooglePerformance extends PerformanceEntity {
  ...
  override def toDocument: Document = {
    ...
    doc ++ super.toDocument
  }

  override def fromDocument(doc: Document){
    super.fromDocument(doc)
    parseDoubleField(doc, costField.fieldName, cost)
    parseIntField(doc, impField.fieldName, impressions)
    parseDoubleField(doc, avgPosField.fieldName, avgPosition)
    parseStringField(doc, dowField.fieldName, dayOfWeek)
    parseStringField(doc, accountField.fieldName, accountName)
    parseIntField(doc, interactionField.fieldName, interactions)
    parseDoubleField(doc, crossDevConvField.fieldName, crossDeviceConversions)
    parseStringField(doc, networkField.fieldName, network)
    parseIntField(doc, gmailClicksField.fieldName, gmailClicksToWebsite)
    parseDoubleField(doc, conversionsField.fieldName, conversions)
    parseIntField(doc, clicksField.fieldName, clicks)
    parseIntField(doc, gmailSavesField.fieldName, gmailSaves)
    parseIntField(doc, gmailForwardsField.fieldName, gmailForwards)
    parseStringField(doc, deviceField.fieldName, device)
    parseDoubleField(doc, viewThroughConvField.fieldName, viewThroughConversions)
    parseDoubleField(doc, revenueField.fieldName, revenue)
    parseIntField(doc, confField.fieldName, conF)
    parseIntField(doc, arrivalsField.fieldName, arrivals)
    parseIntField(doc, conuField.fieldName, conU)
    parseDoubleField(doc, durationField.fieldName, duration)
    parseIntField(doc, lpConvUField.fieldName, lpConvU)
    parseIntField(doc, uFormCompleteField.fieldName, uFormComplete)
    parseIntField(doc, bounceField.fieldName, bounce)
    parseDoubleField(doc, cpcField.fieldName, cpc)
    parseDoubleField(doc, cpmField.fieldName, cpm)
    parseDoubleField(doc, cRateField.fieldName, cRate)
    parseDoubleField(doc, costPerConvField.fieldName, costPerConv)
    parseDoubleField(doc, avgDurationField.fieldName, avgDuration)
    parseDoubleField(doc, lpcRateField.fieldName, lpcRate)
    parseDoubleField(doc, formArrField.fieldName, formArrivals)
    parseDoubleField(doc, formCRateField.fieldName, formCRate)
    parseDoubleField(doc, vplConUField.fieldName, vplConU)
    parseDoubleField(doc, costConUField.fieldName, costConU)
    parseDoubleField(doc, bounceRateField.fieldName, bounceRate)
    parseDoubleField(doc, costLPConvField.fieldName, costLPConv)
    parseDoubleField(doc, ctrField.fieldName, ctr)
  }
}
