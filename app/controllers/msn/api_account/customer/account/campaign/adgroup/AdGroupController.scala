...
class AdGroupController @Inject()(val controllerComponents: ControllerComponents, deadbolt: DeadboltActions, handlers: HandlerCache, actionBuilder: ActionBuilders)(implicit ec: ExecutionContext) extends BaseController with I18nSupport {
  ...
  def adgroups(page: Int, pageSize: Int, orderBy: Int, filter: String) = deadbolt.Dynamic(name=PermissionGroup.MSNRead.entryName, handler=handlers(HandlerKeys.defaultHandler))() {
    implicit request =>
      Future(Ok(views.html.msn.api_account.customer.account_info.campaign.adgroup.adgroups(
        msnAdGroupCollection.find().skip(page * pageSize).limit(pageSize).toList.map(documentToMsnEntity[AdGroup](_, "adGroup", None)),
        page,
        pageSize,
        orderBy,
        filter,
        msnAdGroupCollection.countSync().toInt,
  ...
  def getBid(bid: com.microsoft.bingads.v13.campaignmanagement.Bid): Option[Double] = bid match {
    case x =>
      Some(x.getAmount)
    case _ =>
      None
  ...
  def editAdGroup(id: String) = deadbolt.Dynamic(name=PermissionGroup.MSNWrite.entryName, handler=handlers(HandlerKeys.defaultHandler))() {
    implicit request =>
      msnAdGroupCollection.findOne(Document("adGroupApiId" -> id)) match {
        case Some(x) =>
          val adgroup = documentToMsnEntity[AdGroup](x, "adGroup", None)
          Future(Ok(views.html.msn.api_account.customer.account_info.campaign.adgroup.edit_adgroup(
            ...
            adGroupForm.fill(
              AdGroupForm(
                ...
                // adDistribution, contentMatchBid, and pricingModel were removed in Bing Ads v13
                adDistribution = List.empty,
                adRotationType = adgroup.getAdRotation.toString,
                biddingScheme = adgroup.getBiddingScheme.toString,
                contentMatchBid = None,
        ...
        adgroup => {
          setPendingCache(
            ...
            pendingCache(Left(request)) :+ PendingCacheStructure(
              ...
              changeData = adGroupFormToDocument(adgroup)
            )
          )
